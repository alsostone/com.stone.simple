...
namespace ET
{
    ...
    {
        public static async ETTask<bool> InitPackageAsync(EPlayMode playMode, string packageName)
        {
            // 创建资源包裹类
            ...
            // 编辑器下的模拟模式
            InitializationOperation initializeAsync = null;
            if (playMode == EPlayMode.EditorSimulateMode)
            {
                ...
                initializeAsync = package.InitializeAsync(createParameters);
            }
            ...
            if (initializeAsync.Status != EOperationStatus.Succeed)
            {
                SLogger.Error(initializeAsync.Error);
                return false;
            }
            return true;
        }
