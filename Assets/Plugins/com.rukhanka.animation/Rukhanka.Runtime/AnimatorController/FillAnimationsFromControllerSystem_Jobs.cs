...
/////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	void AnimationsPostSetup(Span<AnimationToProcessComponent> animations, ref LayerBlob lb, int layerIndex, float weightMultiplier, float layerWeight)
	{
		...
		for (int k = 0; k < animations.Length; ++k)
		{
			...
			var isBlobValid = BlobDatabaseSingleton.IsBlobValid(a.avatarMask);
			if (!isBlobValid)
				throw new InvalidOperationException($"Avatar mask blob asset '{lb.avatarMaskBlobHash}' is corrupted. It was likely a part of the subscene data and unloaded.");
		#endif
...
/////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	)
	{
		...
		animationDatabase.TryGetValue(animationHash, out atp.animation);
		
	#if ENABLE_UNITY_COLLECTIONS_CHECKS
		var isBlobValid = BlobDatabaseSingleton.IsBlobValid(atp.animation);
		if (!isBlobValid)
...
/////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	)
	{
		...
		case MotionBlob.Type.BlendTree2DSimpleDirectional:
			blendTreeMotionsAndWeights = StateMachineProcessJob.GetBlendTree2DCurrentMotions(ref mb, runtimeParams, ScriptedAnimator.BlendTree2DType.SimpleDirectional);
			break;
		case MotionBlob.Type.BlendTree2DFreeformCartesian:
			blendTreeMotionsAndWeights = StateMachineProcessJob.GetBlendTree2DCurrentMotions(ref mb, runtimeParams, ScriptedAnimator.BlendTree2DType.FreeformCartesian);
			break;
		case MotionBlob.Type.BlendTree2DFreeformDirectional:
			blendTreeMotionsAndWeights = StateMachineProcessJob.GetBlendTree2DCurrentMotions(ref mb, runtimeParams, ScriptedAnimator.BlendTree2DType.FreeformDirectional);
			break;
		}
